...
```bash
git clone https://github.com/Alex-Nicalace/quotes-cloudfactory-task
cd quotes-cloudfactory-task
```
...
### 5. Запуск на iOS (только macOS)

```bash
npm run ios
```

## Переустановка зависимостей (reinstall)

Для полного удаления и повторной установки всех зависимостей в проекте предусмотрены команды:

`npm run reinstall:win` — для Windows

`npm run reinstall:mac` — для macOS

Команды рассчитаны на выполнение в терминале Bash.
